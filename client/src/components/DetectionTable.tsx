    ...
    <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
      ...
      <div className="overflow-auto max-h-72">
        <table className="w-full text-sm">
          ...
          <tbody>
            {messages.map((m, i) => (
              <tr key={`${m.vehicleId}-${i}`} className="border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                <td className="px-4 py-2 text-slate-300 font-mono text-xs">{m.vehicleId}</td>
                <td className="px-4 py-2">
                  <span className={`px-2 py-0.5 rounded text-xs font-medium capitalize ${CLASS_COLORS[m.class] ?? 'bg-slate-700 text-slate-300'}`}>
                    {m.class}
                  </span>
                </td>
