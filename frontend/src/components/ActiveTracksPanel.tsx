...
const STATUS_ICON: Record<VehicleStatus, React.ReactNode> = {
  active: <Activity   size={12} className="text-green-400" />,
  stale:  <CircleDot  size={12} className="text-slate-400" />,
  lost:   <CircleOff  size={12} className="text-red-400"   />,
};
    ...
    <div
      className={`flex items-center gap-3 px-3 py-2 rounded-lg border transition-colors
        ...
        ${v.status === 'lost'
          ...
          : 'border-slate-700/60 bg-slate-800/40 hover:bg-slate-700/40'
        }`}
    >
      {/* Class icon */}
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          <span className="text-xs font-mono font-semibold text-white">#{v.trackId}</span>
          <span className="text-xs text-slate-400 capitalize">{v.classLabel}</span>
        </div>

        {v.status === 'lost' && (
          <span className="text-xs text-slate-500">{relativeTime(v.lastSeenAt)}</span>
        )}
      </div>

      {/* Speed */}
      {v.status !== 'lost' && (
        <span className="text-xs font-mono font-semibold text-sky-300 shrink-0">
    ...
    <div className={`flex flex-col rounded-2xl border border-slate-700/60 bg-slate-900/60 backdrop-blur shadow-sm overflow-hidden ${className}`}>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700/60 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-white">Active Tracks</h2>
        </div>
        <select
          value={sortKey}
          onChange={e => setSortKey(e.target.value as SortKey)}
          className="text-xs bg-slate-800 border border-slate-700 text-slate-300 rounded px-2 py-1 focus:outline-none"
        >
      ...
      {/* Counts */}
      <div className="flex items-center gap-3 px-4 py-2 border-b border-slate-700/40 text-xs shrink-0">
        <span className="text-green-400 font-semibold">Active: {counts.active}</span>
        <span className="text-slate-400">·</span>
        <span className="text-slate-400">Stale: {counts.stale}</span>
        <span className="text-slate-400">·</span>
        <span className="text-red-400">Lost: {counts.lost}</span>
      </div>

      {/* List — React key={trackId} ensures stable DOM nodes */}
      <div className="flex-1 overflow-y-auto p-2 space-y-1 min-h-0">
        {list.length === 0 ? (
          <p className="text-center text-slate-500 text-sm py-8">No vehicles tracked</p>
        ) : (
